...
- Text is not generated by Jev. Quoted strings and "search for …" phrases in
  the goal are choices. Otherwise Grok (`XAI_API_KEY`, model `grok-4.5` unless
  `--text-model` says otherwise) writes the one string.
...
- Playwright is a package dependency. The Chromium binary is not downloaded
  ...
  install chromium`. CI skips the browser test when that binary is absent.
- Not published to PyPI.
