...
def _presenter(
    *,
    ...
    return EventPresenter(**kwargs), view, status_calls


def test_text_deltas_coalesce_to_one_assistant_paint() -> None:
    scheduled: list = []
...
def test_reasoning_deltas_coalesce_until_flush() -> None:
    scheduled: list = []
...
def test_chrome_does_not_refresh_on_every_token() -> None:
    scheduled: list = []
    presenter, _view, chrome = _presenter(schedule=scheduled)
    presenter.handle("run_started", {"model_id": "openai:test"})
    presenter.handle("turn_started", {"turn": 0})
    chrome.clear()
