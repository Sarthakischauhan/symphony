...
def test_unknown_tool_returns_error_result_instead_of_failing_the_run() -> None:
    ...
    assert _last_terminal(plane.events).event_type == "run_completed"
    assert plane.events[-1].event_type == "run_summary"
...
def test_cancelling_the_run_task_stops_parallel_tools() -> None:
    ...
    assert _last_terminal(plane.events).event_type == "run_cancelled"
    assert plane.events[-1].event_type == "run_summary"
...
def test_max_turns_emits_run_limit_exceeded() -> None:
    ...
    assert _last_terminal(plane.events).event_type == "run_limit_exceeded"
    assert plane.events[-1].event_type == "run_summary"
    assert _last_terminal(plane.events).payload["limit"] == "max_turns"
    _assert_identity(plane.events)
...
def test_max_tool_calls_emits_run_limit_exceeded_and_results_for_all_calls() -> None:
    ...
    assert _last_terminal(plane.events).event_type == "run_limit_exceeded"
    assert plane.events[-1].event_type == "run_summary"
    assert _last_terminal(plane.events).payload["limit"] == "max_tool_calls"
...
def test_max_tokens_emits_run_limit_exceeded() -> None:
    ...
    assert _last_terminal(plane.events).event_type == "run_limit_exceeded"
    assert plane.events[-1].event_type == "run_summary"
    assert _last_terminal(plane.events).payload["limit"] == "max_tokens"
...
def test_max_runtime_stops_stream() -> None:
    ...
    assert _last_terminal(plane.events).event_type == "run_limit_exceeded"
    assert plane.events[-1].event_type == "run_summary"
    assert _last_terminal(plane.events).payload["limit"] == "max_runtime_seconds"
