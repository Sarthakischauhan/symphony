...
`conversation` can include earlier provider messages. `session_id`, `system_prompt`,
and `model_id` can also be overridden per request. Tools stay on the server config.
